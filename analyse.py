...
def run_single(model_params, train_loss, training_data, testing_data, test_output, val_output, results_titles, results, val_results, dir):
    ...
    for i in range(20):
        ...
        plt.close()
        plt.imshow(obs[0,i]-back_out[0,i], vmin=0, vmax=1)
        plt.colorbar()
        plt.savefig(dir + "/test/obs_back_out_"+ str(i) +".png")
        plt.close()
        plt.imshow(obs[0,i]-sig_out[0,i], vmin=0, vmax=1)
        plt.colorbar()
        plt.savefig(dir + "/test/obs_sig_out_"+ str(i) +".png")
        plt.close()
        plt.imshow(sig[0,i]-sig_out[0,i], vmin=0, vmax=1)
        plt.colorbar()
    ...
    for i in range(20):
        ...
        plt.close()
        plt.imshow(val_obs[0,i]-val_back_out[0,i], vmin=0, vmax=1)
        plt.colorbar()
        plt.savefig(dir + "/val/obs_back_out_"+ str(i) +".png")
        plt.close()
        plt.imshow(val_obs[0,i]-val_sig_out[0,i], vmin=0, vmax=1)
        plt.colorbar()
        plt.savefig(dir + "/val/obs_sig_out_"+ str(i) +".png")
        plt.colorbar()
        plt.close()
        plt.imshow(val_sig[0,i]-val_sig_out[0,i], vmin=0, vmax=1)
        plt.colorbar()
